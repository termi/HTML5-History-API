...
;(function( window ){
	...
	if(IELT9_SHIM) {
		var _window_addEventListener = window.addEventListener
		  ...
		  , _Event = window.Event
		  , _window_dispatchEvent = window.dispatchEvent
		  , eventsList = {
				"onpopstate" : [],
				"onhashchange" : []
			}
		...
		if(!_window_addEventListener)window.addEventListener = function(_type, _handler) {
			_type = "on" + _type;
	  ...
	  , hashChanged = (function() {
			...
			fireInitialState = function() {
				if ( initialFire && !( initialFire = 0 ) && normalizeUrl().href !== sets["basepath"] ) {
					...
					setTimeout( _unSafeBind.call(window.dispatchEvent, window, new Event('popstate')) , 10);
				}
			},
